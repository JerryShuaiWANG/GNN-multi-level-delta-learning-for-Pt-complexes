...
def kennardstone(spectra, test_size, metric='euclidean', *args, **kwargs):
...
def main():
    data = pd.read_csv(r"des_total_with_exp_FINAL.csv")
    data = data.drop(columns=["MaxPartialCharge"	,"MinPartialCharge"	,"MaxAbsPartialCharge"	,"MinAbsPartialCharge","BCUT2D_MWHI", "BCUT2D_MWLOW","BCUT2D_CHGHI","BCUT2D_CHGLO","BCUT2D_LOGPHI","BCUT2D_LOGPLOW","BCUT2D_MRHI","BCUT2D_MRLOW"])
    Y = data["Exp. Kr"]
    # X = data.drop(columns = "Hso")
    X = data.iloc[:,0:197]
    print(X)
    # sys.exit()
