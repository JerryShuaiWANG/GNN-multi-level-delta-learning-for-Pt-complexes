...
try:
    ...
    logging.info("Please install apex for mixed precision training from: https://github.com/NVIDIA/apex")
    apex_support = False
...
class MolCLR(object):
    ...
    def train(self):
        ...
        # print("train_loader, valid_loader",train_loader, valid_loader)
        ...
        for epoch_counter in range(self.config['epochs']):
            for bn, (xis, xjs) in enumerate(train_loader):
                ...
                if n_iter % self.config['log_every_n_steps'] == 0:
                    ...
                    logging.info("Epoch %d, step %d, Loss: %f", epoch_counter, bn, loss.item())
            ...
            if epoch_counter % self.config['eval_every_n_epochs'] == 0:
                ...
                logging.info("Epoch %d, step %d, Loss: %f, (validation)", epoch_counter, bn, loss.item())
    ...
    def _load_pre_trained_weights(self, model):
        try:
            ...
            # gcn_pretrained_path = torch.load("/home/shuaiw/DL/PDB_test/ckpt/pretrained_gcn/checkpoints/model.pth", map_location=torch.device('cuda:0'))
            # gin_pretrained_path = "/home/shuaiw/DL/PDB_test/ckpt/pretrained_gin/checkpoints/model.pth"

            # model.load_state_dict(state_dict)
            model.load_state_dict(gcn_pretrained_path)
...
def main():
    ...
    config["model_type"] = 'gcn' # or gin
    config["dataset"]["data_path"] = "./example_unlabelled_strcutures/" #the path for example unlabelled Pt-complex structures (should be 526K unlabelled Pt-complexes structures, files too large thus cannot be fully supplied)
    config["model"]["emb_dim"] = 300  # or 600,  150,  100,  50  (correlated with feat_dim below)
    config["model"]["feat_dim"] = 256 # or 512,  128,  64,   32 

    config['aug'] = 'subgraph' # or node, mix
    config["model"]["drop_ratio"] = 0.05
    logging.info(config)
    if config['aug'] == 'node':
        from dataset.dataset import MoleculeDatasetWrapper
    elif config['aug'] == 'subgraph': # employed in this case
